...
let UpdateGUIFromRunState(pInfo : RunInfo) (m : Model) =
    ...
    let m2 =
        ...
        | PSError TBEXIT
        | PSExit ->
            let m2 = highlightCurrentAndNextIns "editor-line-highlight" (pInfo) m.TabId m
            //enableEditors()
        ...
        | PSError(``Run time error`` (_pos, msg)) ->
            ...
            let m2 = highlightCurrentAndNextIns "editor-line-highlight-error" pInfo m.TabId m
            //updateRegisters()
...
let currentFileTabProgramIsChanged (pInfo : RunInfo) tabId (editors : Map<int, Editor>) =
    let txt = formatText editors.[tabId].IEditor
...
let tryParseAndIndentCode tId (m : Model) =
    ...
    let lim = imageOfTId m.Editors.[tId].IEditor
    ...
    | { Errors = [] } as lim ->
        ...
        let oldCode = List.map trimmed (formatText m.Editors.[tId].IEditor )
...
let runThingOnCode thing editors =
    match Testbench.getTBWithTab editors with
    | Ok(tbTab, _) ->
        match [] |> List.filter (fun id -> id <> tbTab) with //fileTabList
...
let runTests startTest tests stepFunc m =
    ...
    | test :: _ ->
        ...
        | Some(Ok ri) ->
            ...
            let m2 = { m with RunMode = ActiveMode(RunState.Running, ri') }
...
let rec asmStepDisplay (breakc : BreakCondition) (m : Model) steps ri'  =
    ...
    let displayState ri' running =
            ...
            | PSRunning -> // simulation stopped without error or exit
                let m2 = highlightCurrentAndNextIns "editor-line-highlight" ri' m.TabId m 
                let m3 = showInfoFromCurrentMode m2
                let m4 = 
                    if ri.StepsDone < slowDisplayThreshold || (ri.StepsDone - m.LastDisplayStepsDone) > maxStepsBeforeSlowDisplay then
                        let m5 = { m3 with LastDisplayStepsDone = ri.StepsDone }
                        showInfoFromCurrentMode m5
                        else m3
    ...
    | ActiveMode(Stopping, ri') -> // pause execution from GUI button
        ...
        highlightCurrentAndNextIns "editor-line-highlight" ri' m4.TabId m4
    | ResetMode ->
    ...
    | _ -> // actually do some simulation
        ...
        if stepsNeeded <= stepsMax then // in this case we are running to some defined step as part of stepping back, or stepping forward by 1
            ...
            let m5 = { m with RunMode = ActiveMode(Paused, ri') } // mark the fact that we have paused
            ...
            | _ ->
                ...
                let m5 = highlightCurrentAndNextIns "editor-line-highlight" ri' m4.TabId m4
                match handleTest ri' m.Editors with //TODO: CHECK TEST
                | [] -> m5
                | tests -> 
...
let prepareModeForExecution (m : Model) : Model =
    ...
    | RunErrorMode ri
    | ActiveMode(_, ri) ->
        if currentFileTabProgramIsChanged ri m.TabId m.Editors then
    ...
    | FinishedMode ri' ->
        ...
        if currentFileTabProgramIsChanged ri m.TabId m.Editors then
            ...
            if target <= 0L then
                ...
                showInfoFromCurrentMode { m2 with Decorations = newDecorations }
            else
                printfn "Stepping back to step %d" target
                let m3 = { m2 with RunMode = ActiveMode(RunState.Paused, ri) }
                let ri' = asmStep target ri
                let m4 = { m3 with RunMode = ActiveMode(Paused, ri') }
                //TODO: disableEditors()
                let m2 = 
                    match ri'.State with
                    | PSRunning ->
                        highlightCurrentAndNextIns "editor-line-highlight" ri' m.TabId m
...
let runTestbench (m : Model) =
    ...
    | Error(mess) ->
        showVexAlert mess
    | Ok(tabId, tests) when m.TabId = tabId ->
...
let runTestbenchOnCode m =
    runThingOnCode (fun () -> runTestbench m) m.Editors
