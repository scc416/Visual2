...
let createNamedFileTab fName fPath editor =
    ...
    | Some id ->
        ...
        id, None
    | Option.None ->
        let id = uniqueTabId editor //
        let fv =
            DIV [ "editor" ] []
            |> ID(Refs.fileViewIdFormatter id)
        let editor = window?monaco?editor?create (fv)
        ...
        //Refs.editors <- Map.add id editor2 Refs.editors

        // Create the new fileView div
        id, Some editor

//let createFileTab currentFileTabId (editors : Map<int, Editor>) =
    //let id, newTab = createNamedFileTab "Untitled.s" "" editors
    //let newEditor =
    //    match newTab with
    //    | Some x -> 
    //        let newtab = 
